...
<?php 
    ...
    elseif (isset($_POST['addproduct'])){
        ...
        if (move_uploaded_file($_FILES['photo']['tmp_name'], $uploadPath)) {
            if(mysqli_query($conn,"INSERT INTO productlist(prod_name,prod_desc,prod_price) VALUES ('{$_POST['prod_name']}','{$_POST['prod_desc']}',
                {$_POST['prod_price']})")){
                echo "<script>alert('File uploaded successfully.'); window.location.href='listingproductpage.php'</script>";
            }
        }
        else {
            echo "<script>alert('Error uploading file.'); window.location.href='listingproductpage.php'</script>";
        }
    }
    ...
    elseif(isset($_POST['editproduct'])){
        ...
        if($row1 = mysqli_fetch_assoc($result1)){
            if (isset($_FILES['newpicinput']) && $_FILES['newpicinput']['error'] === UPLOAD_ERR_OK) {
                ...
                if ($result2){
                    echo "<script>alert('Edit successful.'); window.location.href='listingproductpage.php';</script>";
                } else{
    ...
    elseif(isset($_GET['delete'])){
        ...
        if($result){
            echo "<script>alert('Hide from user successfully.'); window.location.href='listingproductpage.php'</script>";
        }
        else{
            echo "<script>alert('Hide from user failed.'); window.location.href='listingproductpage.php'</script>";
        }
    }
    elseif(isset($_GET['show'])){
        $result=mysqli_query($conn,"UPDATE productlist SET is_available=1 WHERE prod_id={$_GET['prod_id']}");
        if($result){
            echo "<script>alert('Show to user successfully.'); window.location.href='listingproductpage.php'</script>";
        }
        else{
            echo "<script>alert('Show to user failed.'); window.location.href='listingproductpage.php'</script>";
        }
    }
?>
